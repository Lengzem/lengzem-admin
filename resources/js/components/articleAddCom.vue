<template>
    <div class="container mx-auto p-4">
      ...
      <form @submit.prevent="submitArticle" class="space-y-6">
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
          <!-- MODIFICATION: Left Column - Form Fields (now retractable) -->
          <div v-if="isPanelVisible" class="lg:col-span-4 space-y-6">
            <!-- Title -->
            <div>
          ...
          <!-- MODIFICATION: Right Column - Content Editor (now resizable) -->
          <div
            class="transition-all duration-300 ease-in-out"
            :class="isPanelVisible ? 'lg:col-span-8' : 'lg:col-span-12'"
          >
            <div class="sticky top-4">
              <div class="flex justify-between items-center mb-4">
                <!-- MODIFICATION: Grouped label and toggle button -->
                <div class="flex items-center gap-4">
                  <label for="articleContent" class="block text-sm font-medium text-gray-700 dark:text-gray-300">
                    Content <span class="text-red-500">*</span>
                  </label>
                  <!-- MODIFICATION: Added Toggle Button -->
                  <button
                    type="button"
                    @click="isPanelVisible = !isPanelVisible"
                    class="flex items-center gap-1.5 text-xs text-gray-500 hover:text-gray-800 dark:text-gray-400 dark:hover:text-gray-200 focus:outline-none p-1 rounded-md hover:bg-gray-100 dark:hover:bg-gray-700"
                    :title="isPanelVisible ? 'Hide Details Panel' : 'Show Details Panel'"
                  >
                    <ChevronDoubleLeftIcon v-if="isPanelVisible" class="h-4 w-4" aria-hidden="true" />
                    <ChevronDoubleRightIcon v-else class="h-4 w-4" aria-hidden="true" />
                    <span class="hidden sm:inline">{{ isPanelVisible ? 'Hide Panel' : 'Show Panel' }}</span>
                  </button>
                </div>
  ...
  // MODIFICATION: Import new icons for the toggle button
  import { PlusIcon, ExclamationCircleIcon, ChevronDoubleLeftIcon, ChevronDoubleRightIcon } from '@heroicons/vue/24/outline';
  import axios from 'axios';
  import { useToast } from 'vue-toastification';

  const toast = useToast();
  // MODIFICATION: Add state for panel visibility
