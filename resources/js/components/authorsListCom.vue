<template>
  <div class="max-w-10xl px-4 sm:px-6 lg:px-8 py-8">
    <div class="bg-white dark:bg-gray-900 rounded-xl shadow-lg overflow-hidden border border-gray-200 dark:border-gray-800 transition-all duration-300">
              ...
              <tbody class="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
                <tr v-for="author in authors" :key="author.id" class="hover:bg-gray-50 dark:hover:bg-gray-800/70 transition-colors duration-150">
                  <td class="px-6 py-4 whitespace-nowrap">
                    <div class="flex items-center">
                      <div class="flex-shrink-0 h-10 w-10">
                        <img class="h-10 w-10 rounded-full object-cover" :src="author.user.profile_image_url || 'https://ui-avatars.com/api/?name=' + encodeURIComponent(author.pen_name || 'A') + '&color=7F9CF5&background=EBF4FF'" :alt="author.pen_name + ' avatar'">
                      </div>
                      <div class="ml-4">
                  ...
                  <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                    <button @click="openAuthorViewModal(author.id)" type="button" class="text-indigo-600 hover:text-indigo-800 dark:text-indigo-400 dark:hover:text-indigo-300 transition-colors font-medium">View</button>
                    <button @click="openAuthorEditModal(author.id)" type="button" class="text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300 transition-colors font-medium">Edit</button>
                    <button @click="openConfirmDeleteModal(author.id)" type="button" class="text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-300 transition-colors font-medium">Delete</button>
                  </td>
                </tr>
              </tbody>
            </table>
     ...
     <AuthorDetailModal 
        :is-visible="showAuthorViewModal" 
        :author-id="viewingAuthorId" 
        @close="closeAuthorViewModal" 
      />
      
      <!-- MODIFIED: Pass author-id prop -->
  <AuthorEditModal 
    :is-visible="showAuthorEditModal" 
    :author-id="editingAuthorId"
    @close="closeAuthorEditModal"
                ...
                <div class="bg-white dark:bg-gray-800 px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                  <div class="sm:flex sm:items-start">
                    ...
                    <div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                      <h3 class="text-base font-semibold leading-6 text-gray-900 dark:text-gray-100" id="modal-title">Delete Author</h3>
                      <div class="mt-2">
                        <p class="text-sm text-gray-500 dark:text-gray-400">Are you sure you want to delete this author? This action cannot be undone.</p>
                      </div>
                    </div>
                  </div>
                </div>
...
const fetchAuthors = async (page = 1) => {
  ...
  try {
    ...
    if (err.response?.status === 401) {
      authorsError.value = "You are not authorized. Please log in again.";
    } else {
