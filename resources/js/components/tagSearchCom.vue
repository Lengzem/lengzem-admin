...
const performSearch = async () => {
    ...
    if (!query) {
      searchResults.value = [];
      searchPerformed.value = true; // Show "No results" if submitted empty
      return;
    }
    ...
    try {
        const response = await axios.get(`https://api.lengzem.in/api/tags/search?name=${encodeURIComponent(query)}`);
...
const handleTagSaved = (newTag) => {
  ...
  // Optional: refresh your main list of tags if one is displayed
  // Optional: clear search and show the new tag
  searchQuery.value = newTag.name;
  performSearch(); // Re-run search to show the newly created tag
};
</script>