    ...
    <div class="bg-white dark:bg-gray-900 rounded-xl shadow-lg overflow-hidden border border-gray-200 dark:border-gray-800 transition-all duration-300">
      ...
      <div class="p-4 sm:p-6">
        <div class="bg-white dark:bg-gray-900 rounded-lg shadow-md overflow-hidden border border-gray-200 dark:border-gray-700">
          <!-- ... (loading, error, no users states - same as before) ... -->
          ...
          <!-- Pagination (if you have it, same as CategoryList) -->
        </div>
      </div>
    </div>
...
<script setup>
import { ref, onMounted, reactive } from 'vue'; // Added reactive for pagination if used
import axios from 'axios';
import UserDetailModal from './userDetailCom.vue'; // Import the modal
...
// Basic pagination state (adapt if your API uses it for users list)
...
const fetchUsers = async (page = 1) => {
  ...
  try {
    ...
    if (response.data && response.data.status === true && response.data.data && Array.isArray(response.data.data.data)) {
      ...
      // Update pagination if API returns it:
...
const roleClass = (role) => { // Helper for styling role badges in table
