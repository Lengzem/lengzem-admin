<template>
          ...
          >
            <form @submit.prevent="handleSubmit" class="relative bg-white dark:bg-gray-900 w-full max-w-lg mx-auto rounded-xl shadow-2xl overflow-hidden border border-gray-200 dark:border-gray-700">
              ...
              <div class="flex items-center justify-between p-5 border-b border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-800/50">
                <h2 class="text-xl font-semibold text-gray-800 dark:text-gray-100">Edit Tag</h2>
                <button 
                  @click="$emit('close')" 
                  type="button" 
              ...
              <div class="p-6 space-y-6 max-h-[70vh] overflow-y-auto">
                ...
                <div v-if="loading" class="text-center py-10">
                  <div class="inline-flex items-center justify-center">
                    ...
                    <span class="ml-3 text-gray-600 dark:text-gray-400">Loading tag data...</span>
                  </div>
                </div>
                ...
                <div v-else class="space-y-6">
                  <!-- Tag Name -->
                  <div>
                    <label for="tag_name" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Tag Name</label>
                    <input 
                  ...
                  <div>
                    ...
                    <input 
                      ...
                      id="tag_slug" 
                      class="w-full px-4 py-2.5 rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all"
                      placeholder="e.g., unique-tag-slug"
                    >
                  </div>
                </div>
  ...
  const fetchTagDetails = async (id) => {
    ...
    formData.value = { name: '', slug: '' }; // Reset form data
  
    try {
      const response = await axios.get(route('proxy.get', { endpoint: `tags/${id}` }));
      ...
      console.error("Failed to fetch tag for editing:", err);
      fetchError.value = "Could not load tag details. Please close and try again.";
    } finally {
      loading.value = false;
    }
  };
  ...
  watch(() => props.isVisible, (newValue) => {
    if (newValue && props.tagId) {
      fetchTagDetails(props.tagId);
    }
  });
  ...
  const handleSubmit = async () => {
    ...
    try {
      await axios.put(route('proxy.put'), formData.value, {
          params: { endpoint: `tags/${props.tagId}` }
      });
      
      toast.success('Tag updated successfully!');
      emit('saved');
    } catch (err) {
      saveError.value = err.response?.data?.message || "An error occurred while saving the tag.";
